...
- 🕵️ **Constancia en cada "a cuenta"**: se guarda automáticamente **quién** la registró, **en qué día real** y **a qué hora** lo hizo (se ve en la ficha y en el formulario de edición). Si la fecha del pago no coincide con el día en que se registró, aparece un **⚠️** para que lo revises
...
- 🧾 **Cobro en la calle**: desde la ficha, el empleado registra el pago eligiendo **Efectivo / Yape / BCP**; la **fecha es siempre la de hoy y no se puede cambiar**. Un botón permite **saldar de una vez todo lo que debe**
...
- 👥 **Usuarios y permisos**: solo el administrador crea usuarios (con contraseña, sin correo); a cada uno le das o le quitas permisos (crear, editar, registrar pagos, **borrar**, ver/exportar cobranza, registrar/editar clientes, **crear la hoja de cobranza del día**, **cambiar la fecha de vencimiento**, **armar despachos de reparto**). El acceso a los datos y el borrado quedan blindados en la base de datos
- 🧭 **Panel lateral en computadora**: en pantallas anchas aparece un menú a la izquierda (Créditos, Despachos, Clientes, Hoja de cobranza, Usuarios, Configuración). En celular y tablet se mantienen los mismos botones arriba
