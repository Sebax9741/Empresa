...
- 🛒 **Productos** (botón 🛒): catálogo con **código de producto** (PR-0001, PR-0002… se genera solo), **nombre**, **presentación** (Balde, Caja, Saco, Paquete o Unidad) y **tres precios: A, B y C**. Al crearlo se anota el **stock inicial** (lo que hay hoy en el almacén) y su **stock mínimo**, para que la app avise cuando se está acabando. Un producto que ya no vendes se marca **inactivo**: deja de salir al vender pero conserva todo su historial. El botón **📥 Stock** de cada producto es donde se anota que **llegó mercadería** o se **cuadra con el conteo físico**
- 📒 **Kardex de almacén** (botón 📒): la hoja **se llena sola** —cada nota de venta descuenta lo vendido y cada producto nuevo entra con su stock inicial—, así que ahí no hay que escribir nada: es la hoja que se consulta y se imprime. Cada línea anota **fecha y hora, producto, tipo (📥 entrada / 📤 salida / ⚖️ ajuste), motivo**, **documento de referencia** (nº de factura o de nota de venta), **cuánto entró o salió, el saldo que quedó** y **quién lo registró**. El stock **nunca se escribe a mano**: sale de sumar el kardex, así que siempre se puede auditar fila por fila. En un **ajuste** escribes lo que contaste de verdad en el almacén y la app calcula sola la diferencia. Se filtra por producto, tipo y rango de fechas
...
- 👥 **Usuarios y permisos**: solo el administrador crea usuarios (con contraseña, sin correo); a cada uno le das o le quitas permisos (crear, editar, registrar pagos, **borrar**, ver/exportar cobranza, registrar/editar clientes, **crear la hoja de cobranza del día**, **armar despachos de reparto**, **emitir notas de venta**, **gestionar productos y almacén**). El acceso a los datos y el borrado quedan blindados en la base de datos. Un empleado que ya podía crear créditos **puede vender**, pero **tocar el catálogo y el almacén hay que dárselo a propósito**
- 🧭 **Panel lateral en computadora**: en pantallas anchas aparece un menú a la izquierda (Créditos, Notas de venta, Productos, Kardex, Despachos, Clientes, Hoja de cobranza, Usuarios, Configuración). En celular y tablet se mantienen los mismos botones arriba
