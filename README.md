...
- 🎁 **Bonificaciones**: cualquier línea se convierte en regalo con un toque. El producto **conserva su precio a la vista** —para que quede claro qué se está regalando y por cuánto— y en la columna **Dscto. bonif.** se le resta ese mismo importe, así que **la línea queda en cero**. El subtotal la sigue contando y el total ya no la cobra. En el comprobante impreso la columna **solo aparece cuando hay alguna**: una nota normal no gasta ancho en una columna de guiones
- ✏️ **Modificar una nota emitida**: se abre con su mismo número y su misma hora —es la misma nota corregida, no una copia— y al guardar **el almacén se recalcula solo**: se retiran del kardex las salidas viejas y se anotan las nuevas. Si esa nota ya tenía **despacho** o **crédito**, se les pasa el importe nuevo, para que no queden diciendo lo que la nota decía antes. Una nota cuyo crédito **ya tiene cobros registrados no se puede modificar**: ahí el importe ya no manda la nota
...
- 🗑️ **Eliminar una nota**: esto sí borra —la nota desaparece y no queda rastro de que existió—, así que es solo del **administrador** y **pide el código de seguridad**. Deshace lo mismo que anular, pero sin dejar constancia. Para el día a día está **🚫 Anular**
...
- 🗑️ **Borrar una nota devuelve su mercadería**: el administrador puede borrarla (pide el código de seguridad); se anulan sus salidas de almacén y el stock vuelve a estar disponible. Una nota que **ya tiene crédito no se borra**: primero hay que borrar el crédito. En el **Kardex** ya se puede anular **cualquier** movimiento, también las salidas por venta; si la salida es de una nota, el aviso lo dice y recuerda que lo normal es borrar la nota entera, que devuelve todo de una vez
