...
- 📥 **Ingreso de productos** (botón 📥): la sección donde entra la mercadería. Eliges el **producto**, la **cantidad que ingresa** y la **fecha**, y la app te muestra al instante el **stock actual** y **cuánto va a quedar** antes de guardar nada. Sirve también para **corregir el stock con el conteo físico** (escribes lo contado y la app calcula sola la diferencia) o anotar una salida que no fue venta (merma, traslado). Cada movimiento admite **proveedor / documento** y una nota, y abajo se ve el historial de lo registrado. El botón 📥 de cada fila en Productos abre esta sección con ese producto ya elegido
...
- 👥 **Usuarios y permisos**: solo el administrador crea usuarios (con contraseña, sin correo); a cada uno le das o le quitas permisos (crear, editar, registrar pagos, **borrar**, ver/exportar cobranza, registrar/editar clientes, **crear la hoja de cobranza del día**, **armar despachos de reparto**, **emitir notas de venta**, **gestionar productos y almacén**). El acceso a los datos y el borrado quedan blindados en la base de datos. Un empleado que ya podía crear créditos **puede vender**, pero **tocar el catálogo, ingresar mercadería y corregir el stock hay que dárselo a propósito**
- 🧭 **Panel lateral en computadora**: en pantallas anchas aparece un menú a la izquierda (Créditos, Notas de venta, Productos, Ingreso de productos, Kardex, Despachos, Clientes, Hoja de cobranza, Usuarios, Configuración). En celular y tablet se mantienen los mismos botones arriba
